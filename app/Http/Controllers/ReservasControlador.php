...
{
    public function mostrarMisReservas(Request $request)
    {
        // Obtener el ID del usuario autenticado
        $clienteID = Auth::user()->id;

        // Recuperar todas las reservas del usuario autenticado con sus servicios adicionales
        $reservas = DB::table('reservas') //Selecciona la tabla reservas
            ...
            ->select(
                ...
                'users.name as cliente_nombre' //Selecciona el nombre del cliente que ha hechi la reserva
            )
            ->where('reservas.clienteID', $clienteID) // Filtra las reservas para que solo incluya las que pertenecen al cliente autenticado
