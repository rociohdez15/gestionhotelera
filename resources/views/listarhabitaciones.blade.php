        ...
        <div class="container">
            ...
            <form action="{{ route('buscarHabitaciones') }}" method="GET" class="mb-3">
                <div class="input-group">
                    <input type="text" class="form-control" name="query" placeholder="Buscar por ID, nº habitacion o precio" aria-label="Buscar reservas">
                        ...
                        function actualizarTabla(page = 1, query = '') {
                            $.ajax({
                                ...
                                success: function(response) {
                                    ...
                                    tabla.empty();
                                    $.each(response.data, function(index, habitacion) {
                                        var pdfUrl = '{{ route("generar_pdf_listar_habitaciones", ":habitacionID") }}';
                                        pdfUrl = pdfUrl.replace(':habitacionID', habitacion.habitacionID);
                                        ...
                                        tabla.append(
                                            ...
                                            '</div>' +
                                            '<div class="modal-body">¿Estás seguro de que deseas eliminar esta habitacion?</div>' +
                                            ...
                                            '<div class="modal-content">' +
                                            '<div class="modal-header">' +
                                            '<h5 class="modal-title" id="confirmEditModalLabel' + habitacion.habitacionID + '">Confirmar editar</h5>' +
                                            '<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>' +
                                            '</div>' +
                                            '<div class="modal-body">¿Estás seguro de que deseas editar esta habitacion?</div>' +
                                            '<div class="modal-footer">' +
